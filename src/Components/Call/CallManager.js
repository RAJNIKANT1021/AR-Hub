 ...
 *
 * Provides:
 *   useCallManager()  → { startCall, answerCall, hangup, incomingCall, activeCall }
 *
 * Renders:
 *   <IncomingCallCard> when there's an incoming offer
 *   <CallScreen>       when a call is active
 */
...
export function CallProvider({ uid, myName, myAvatar, getUser, children }) {
  const [incomingCall, setIncomingCall]   = useState(null);  // raw Firestore doc
  const [activeCall,   setActiveCall]     = useState(null);  // { type, partner, cid, ... }
  ...
  const callStartRef = useRef(null); // Date.now() when call connected

  // ── Listen for incoming calls ──────────────────────────────
  useEffect(() => {
    if (!uid) return;
    return subscribeIncomingCall(uid, async (callDoc) => {
      if (!callDoc) { setIncomingCall(null); return; }
  ...
  // ── Shared cleanup ─────────────────────────────────────────
  const doHangup = useCallback(async (cid, shouldDeleteDoc = true, callMeta = null) => {
    const duration = callStartRef.current
      ? Math.round((Date.now() - callStartRef.current) / 1000)
      : 0;
    callStartRef.current = null;

    // Write log for both participants
    if (callMeta && uid) {
      const { partner, callType, direction } = callMeta;
      const logEntry = {
        cid,
      ...
      writeCallLog(uid, logEntry).catch(() => {});
    }
  ...
  const initiateCall = useCallback(async (partner, type = "video") => {
  ...
  const acceptCall = useCallback(async () => {
    ...
    const { id: cid, callType: type, callerId, callerName, callerAvatar } = incomingCall;
  ...
  const declineCall = useCallback(async () => {
    if (!incomingCall) return;
    setIncomingCall(null);
    // Mark call as ended so caller UI updates
  ...
  return (
    <CallCtx.Provider value={{ initiateCall, hangup, incomingCall, activeCall, isConnected }}>
      {children}
